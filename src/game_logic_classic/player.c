...
static void UpdatePlayer_Shooting(Context* ctx)
{
  ...
  //
  // [BUG] Because ctx->plRapidFireIsActiveFrame isn't reset when picking up the
  ...
  // Depending on the state of ctx->plRapidFireIsActiveFrame, the player starts
  // shooting immediately, or one frame later.
  // One way to fix this would be to reset ctx->plRapidFireIsActiveFrame to zero
